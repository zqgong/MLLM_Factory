...
@dataclass
class GKDDataCollatorWithPadding(MultiModalDataCollatorForSeq2Seq):
    block_diag_attn: bool = False
    attn_implementation: Literal["eager", "sdpa", "flash_attention_2"] = "eager"
    compute_dtype: "torch.dtype" = torch.float32
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, "torch.Tensor"]:
        ...
        prompts = np.array(torch.where(labels == IGNORE_INDEX, input_ids, self.tokenizer.pad_token_id))
        prompts_input_ids = [prompt[prompt != self.tokenizer.pad_token_id] for prompt in prompts]
        prompt_attention_mask = [np.ones_like(i) for i in prompts_input_ids]
        ...
        prompts_input_ids = self.pad(prompts_input_ids, padding_side="left", padding_value=self.tokenizer.pad_token_id)
        prompt_attention_mask = self.pad(prompt_attention_mask, padding_side="left", padding_value=0)

        features["prompts"] = prompts_input_ids
        features["prompt_attention_mask"] = prompt_attention_mask
